...
test.describe.serial('cash session', () => {
  test.beforeEach(({}, testInfo) => {
    test.skip(
      ...
      'Cash session tests mutate the shared cash_sessions row and run only in the stateful project',
    );
  });

  test('opens a session, records a movement, and closes it back', async ({ page }) => {
  ...
  test('opens a session, starts closing, cancels, and closes it back', async ({ page }) => {
    await loginByPin(page, employees.carlos);
    await expect(page).toHaveURL(/\/app\/caja$/);
